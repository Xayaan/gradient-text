...
RUN uv pip install packaging setuptools wheel awscli pydantic \
      ...
      PyYAML textstat langcheck detoxify RestrictedPython \
      accelerate bitsandbytes deepspeed typer tensorboard \
      redis liger-kernel astor \
...
RUN chmod +x /workspace/scripts/run_text_trainer.sh /workspace/scripts/entrypoint.sh /workspace/scripts/text_trainer.py
