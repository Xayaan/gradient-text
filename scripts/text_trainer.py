...
MODEL_ARTIFACT_SUFFIXES = (".safetensors", ".bin", ".pt")
...
def directory_has_model_artifacts(path: str) -> bool:
    ...
    for entry in os.listdir(path):
        ...
        if not os.path.isfile(full_path):
            continue
        if entry in MODEL_ARTIFACT_FILENAMES:
            return True
        if entry.endswith(MODEL_ARTIFACT_SUFFIXES):
            return True
    return False


def output_has_model_artifacts(output_dir: str) -> bool:
    if directory_has_model_artifacts(output_dir):
        return True
    if not os.path.isdir(output_dir):
        return False
    for entry in os.listdir(output_dir):
        candidate = os.path.join(output_dir, entry)
        if os.path.isdir(candidate) and entry.startswith("checkpoint-") and directory_has_model_artifacts(candidate):
            return True
    return False


def patch_model_metadata(output_dir: str, base_model_id: str):
    metadata_files = ("adapter_config.json", "config.json")
    for filename in metadata_files:
        ...
        try:
            ...
            print(f"Skipping metadata patch for {metadata_path}: {exc}", flush=True)
            continue

        changed = False
        if "base_model_name_or_path" in data and data["base_model_name_or_path"] != base_model_id:
            data["base_model_name_or_path"] = base_model_id
            changed = True
        if "_name_or_path" in data and isinstance(data["_name_or_path"], str):
            existing = data["_name_or_path"]
            if existing != base_model_id and (
                existing.startswith("/cache/") or existing.startswith("/workspace/") or existing.startswith("/app/")
            ):
                data["_name_or_path"] = base_model_id
                changed = True
        ...
        if changed:
            try:
                ...
                print(f"Patched model metadata: {metadata_path}", flush=True)
            except Exception as exc:
                print(f"Failed to write patched metadata {metadata_path}: {exc}", flush=True)


def copy_base_model_snapshot(base_model_path: str, output_dir: str) -> bool:
    ...
    for name in os.listdir(base_model_path):
        ...
        try:
            ...
            print(f"Failed to copy fallback model asset {source}: {exc}", flush=True)
    return copied_any


def ensure_submission_model(base_model_path: str, output_dir: str) -> bool:
    if output_has_model_artifacts(output_dir):
        return True
    print("No model artifacts found after training; attempting base-model snapshot fallback.", flush=True)
    if copy_base_model_snapshot(base_model_path, output_dir):
        return output_has_model_artifacts(output_dir)
    return False
...
def main():
    ...
    if not args.expected_repo_name:
        args.expected_repo_name = f"submission-{args.task_id}"
    ...
    if (
        ...
        tokenize_python = "/workspace/axo_py/bin/python" if os.path.exists("/workspace/axo_py/bin/python") else "python"
        tokenize_cmd = f"{tokenize_python} tokenize_instruct.py {request_path}"
    ...
    while True:
        ...
        if args.task_type == TaskType.GRPOTASK.value:
            ...
            if state["mode"] == "initial":
                ...
                n_runs = max(1, int(state.get("next_runs", 1)))
                if "lrs" not in state: # first time of continue
                    current_lr = float(state.get("train", {}).get("lr", extract_value_from_cmd(train_cmd, "learning_rate") or 0))
                ...
                set_state(state)
                current_train = state.get("train", {}).copy()
                current_train.setdefault("current_loss", float("inf"))
                state["runs"].append(current_train)
                ...
                if len(state["runs"]) < n_runs:
                    ...
                    train_cmd = state["runs"][index].get("train_cmd", train_cmd)
                    final_output_dir = state["runs"][index].get("output_dir")
                    state["mode"] = "finish"
    ...
    if not output_has_model_artifacts(submission_dir):
        ensure_submission_model(model_path, submission_dir)
    patch_model_metadata(submission_dir, original_model_name)
